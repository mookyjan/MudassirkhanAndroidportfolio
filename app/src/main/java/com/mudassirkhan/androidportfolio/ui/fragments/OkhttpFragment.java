...
public class OkhttpFragment extends BaseMovieRecyclerViewFragment {
    ...
    public String loaderBackgroundAction() {
        String url = NetworkUtils.getCustomMovieUrl(getActivity(), 10751);
        String data = null;
        try {
            data = NetworkUtils.getJsonStringThroughOkhttp(url);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return data;
